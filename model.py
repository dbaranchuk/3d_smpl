...
class _3DINN(object):
    ...
    def loadData(self):
        ...
        surreal_test_filenames = ["../tf_code/gait/surreal/prev/test.tfrecords"]
            ...
            while not coord.should_stop():
                for idx in xrange(0, self.config.max_iter):
                    ...
                    if idx % 100 == 0:
                        ...
                        self.writer.add_summary(summ_str, step)
                        print("[%s, iter: %d] Losses: sup: %.4f, d3: %.4f (%.6f) (%.4f), d2: %.4f (%.6f), pixel: %.4f, silh: %.4f, beta: %.4f, pose: %.4f, R: %.4f, T: %.4f" %(self.config.name, idx, sup_loss, d3_joint_loss, d3_loss, d3_c_loss, d2_joint_loss, d2_loss, silh_loss, beta_loss, pose_loss, R_loss, T_loss))
